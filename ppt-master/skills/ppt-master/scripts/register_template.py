...
def _parse_simple_yaml_scalar(value: str) -> object:
    """Parse the small YAML subset used by template frontmatter."""
    value = value.strip()
    if not value:
        return ""
    if value.startswith("[") and value.endswith("]"):
        try:
            ...
            raise SpecParseError(f"invalid inline list in frontmatter: {value}") from exc
        if not isinstance(parsed, list):
            raise SpecParseError(f"inline frontmatter value must be a list: {value}")
        return parsed
...
def _parse_simple_yaml_frontmatter(fm_block: str) -> dict:
    """Fallback parser for dependency-free template frontmatter.

    It intentionally supports only the subset used by this template library:
    top-level scalars, top-level lists, nested maps, nested lists, and inline
    Python/YAML-style string lists.
    """
    ...
    for raw_line in fm_block.splitlines():
        ...
        if indent == 0:
            if ":" not in line:
                raise SpecParseError(f"invalid frontmatter line: {raw_line}")
            key, value = line.split(":", 1)
            current_key = key.strip()
            current_map_key = None
            value = value.strip()
            data[current_key] = [] if value == "" else _parse_simple_yaml_scalar(value)
            continue

        if current_key is None:
            raise SpecParseError(f"nested frontmatter line without parent: {raw_line}")

        if isinstance(data.get(current_key), dict) and current_map_key and line.startswith("- "):
            nested = data[current_key]
            assert isinstance(nested, dict)
            seq = nested.setdefault(current_map_key, [])
            if not isinstance(seq, list):
                raise SpecParseError(
                    f"frontmatter key {current_key}.{current_map_key} mixes list and scalar values"
                )
            seq.append(_parse_simple_yaml_scalar(line[2:].strip()))
            continue

        if line.startswith("- "):
            seq = data.get(current_key)
            if not isinstance(seq, list):
                data[current_key] = seq = []
            seq.append(_parse_simple_yaml_scalar(line[2:].strip()))
            continue
        ...
        if ":" in line:
            ...
            current_map_key = subkey.strip()
            value = value.strip()
            nested[current_map_key] = [] if value == "" else _parse_simple_yaml_scalar(value)
            continue

        raise SpecParseError(f"unsupported frontmatter line: {raw_line}")
...
def _read_spec(spec_path: Path) -> tuple[dict | None, str]:
    ...
    body = text[end + 5:]
    if yaml is None:
        return _parse_simple_yaml_frontmatter(fm_block), body
    try:
